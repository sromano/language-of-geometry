...
min_prog x n = fromJust ((min_prog' (substringChunks x) emptyDict) (x,n))
...
min_prog_repeat y n dict = [[p:^: ((length y) `div` d)] | d <- [1..(length y)-1], isJust (dict ((take d y),n)), p <- fromJust (dict ((take d y),n)), (length y) `mod` d == 0, y == (exe [p:^: ((length y) `div` d)] n)]

min_prog_C y n dict =[[p :*: (((length y) `div` d), i)] | d <- [1..(length y)-1], [i] <- atomic_progs, isJust (dict ((take d y),n)), p <- (fromJust (dict ((take d y),n))), (length y) `mod` d == 0, y == (exe [p :*: (((length y) `div` d), i)] n) ]

min_prog_D y n dict =[[p :+: (((length y) `div` d), i)] | d <- [1..(length y)-1], [i] <- atomic_progs, isJust (dict ((take d y),n)), p <- (fromJust (dict ((take d y),n))), (length y) `mod` d == 0, y == (exe [p :+: (((length y) `div` d), i)] n) ]
