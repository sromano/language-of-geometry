module BinaryLot(min_progs_absolute_B, complexity_B) where
...
min_progs_absolute_B (x:xs) = (x:xs) ++ " ; complexity = " ++ (show len) ++ " \n " ++ concat [(show q) ++ " \n " | q <- ps]
                            where len = size (ps !! 0)
...
complexity_B (x:xs) = (x:xs) ++ " ; " ++ (show len) ++ " ; " ++ (show (length ps))
                            where len = size (ps !! 0)
                                  ps  = min_prog (x:xs) (digitToInt x)
